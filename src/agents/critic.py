...
Provide your critique."""
    ...
    def _parse_critique(self, response: str) -> Dict[str, str]:
        ...
        # Fallback: some models ignore the requested headers entirely.
        if not critique["assessment"] and not critique["issues"] and not critique["suggestions"]:
            cleaned = " ".join((response or "").split()).strip()
            if cleaned:
                critique["assessment"] = cleaned[:500] + ("..." if len(cleaned) > 500 else "")
